...
var GlAccountService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.GlAccountService = exports.IFB_GL_DATA = exports.CONVENTIONAL_GL_DATA = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const gl_account_entity_1 = require("../entities/gl-account.entity");
const XLSX = __importStar(require("xlsx"));
exports.CONVENTIONAL_GL_DATA = [
...
let GlAccountService = GlAccountService_1 = class GlAccountService {
    glRepo;
    logger = new common_1.Logger(GlAccountService_1.name);
    constructor(glRepo) {
        this.glRepo = glRepo;
    }
    async onModuleInit() {
    ...
    async seedDefaultGls() {
        ...
        let seededCount = 0;
        let updatedCount = 0;
        const validConventionalCodes = new Set(exports.CONVENTIONAL_GL_DATA.map((d) => d.glCode));
        const validIfbCodes = new Set(exports.IFB_GL_DATA.map((d) => d.glCode));
        for (const item of exports.CONVENTIONAL_GL_DATA) {
            const existing = await this.glRepo.findOne({ where: { glCode: item.glCode, bankingType: gl_account_entity_1.BankingType.CONVENTIONAL } });
            if (!existing) {
                await this.glRepo.save(this.glRepo.create({
                    glCode: item.glCode,
                    glDescription: item.glDescription,
                    categoryGroup: item.categoryGroup,
                    bankingType: gl_account_entity_1.BankingType.CONVENTIONAL,
                    isActive: true,
                }));
                seededCount++;
            }
                ...
                existing.bankingType = gl_account_entity_1.BankingType.CONVENTIONAL;
                existing.isActive = true;
                await this.glRepo.save(existing);
                updatedCount++;
            }
        }
        for (const item of exports.IFB_GL_DATA) {
            const existing = await this.glRepo.findOne({ where: { glCode: item.glCode, bankingType: gl_account_entity_1.BankingType.IFB } });
            if (!existing) {
                await this.glRepo.save(this.glRepo.create({
                    glCode: item.glCode,
                    glDescription: item.glDescription,
                    categoryGroup: item.categoryGroup,
                    bankingType: gl_account_entity_1.BankingType.IFB,
                    isActive: true,
                }));
                seededCount++;
            }
                ...
                existing.glDescription = item.glDescription;
                existing.categoryGroup = item.categoryGroup;
                existing.bankingType = gl_account_entity_1.BankingType.IFB;
                existing.isActive = true;
                await this.glRepo.save(existing);
                updatedCount++;
            }
        ...
        for (const gl of allDbGls) {
            const isValidConventional = gl.bankingType === gl_account_entity_1.BankingType.CONVENTIONAL && validConventionalCodes.has(gl.glCode);
            const isValidIfb = gl.bankingType === gl_account_entity_1.BankingType.IFB && validIfbCodes.has(gl.glCode);
    ...
    async findAll(bankingType, search) {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            try {
                ...
                if (existing) {
                    ...
                    if (categoryGroup)
                        existing.categoryGroup = categoryGroup;
                    await this.glRepo.save(existing);
                    updated++;
                }
                else {
...
exports.GlAccountService = GlAccountService;
exports.GlAccountService = GlAccountService = GlAccountService_1 = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(gl_account_entity_1.GlAccount)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], GlAccountService);
//# sourceMappingURL=gl-account.service.js.map